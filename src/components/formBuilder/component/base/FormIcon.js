...
export default class FormIcon extends PureComponent {
    render() {
        ...
        let classess = this.props.className ? "icon-wrap " +  this.props.className : "icon-wrap";
        switch(icon) {
            case 'text': {
        ...
        return (
            <span className={classess} onClick={this.props.onClick}>
                <i className="form-icon text" style={{backgroundImage: `url(${name+".png"})`, width: `${width}px`}}></i>
                <i className="form-icon text" style={{backgroundImage: `url(${hoverName})`, width: `${width}px`}}></i>
            </span>
        )
    }
}
