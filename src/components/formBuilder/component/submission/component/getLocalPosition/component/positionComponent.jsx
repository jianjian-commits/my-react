...
export default class PositionComponent extends Component {
  ...
  getAddress = () => {
    this.geocoder.getAddress(this.lnglat, (status, result) => {
      if (status === 'complete' && result.regeocode) {
        ...
        this.props.onChange({ address, latitude: this.lnglat.lat, longitude: this.lnglat.lng });
      } else {
        this.setState({
          address,
          errorMsg: "查询地址失败"
  ...
  showPositionList = () => {
    ...
    AMap.service(["AMap.PlaceSearch"], () => {
      ...
      AMap.event.addListener(placeSearch, 'listElementClick', this.clickPositionItem);
      AMap.event.addListener(placeSearch, 'markerClick', this.clickPositionMarker);
      placeSearch.searchNearBy('', this.lnglat, rangeNumber, (status, result) => {
  ...
  clickPositionItem = (data) => {
    ...
    this.setState({  //修改后的位置
      changedAdress: address,
      changedLatitude: location.lat,
      changedLongtitude: location.lng,
    }, () => {
      this.props.onChange({ address, latitude: location.lat, longitude: location.lng });
    })
  }
  ...
  clickPositionMarker = (e) => {
    ...
    this.setState({  //修改后的位置
      ...
      this.props.onChange({ address:e.data.address, latitude: e.data.location.lat, longitude:  e.data.location.lng })
    })
    ;
  }
  showMap = () => {
  ...
  renderAjustmentRangePositionNode = () => {
    let address = this.state.changedLatitude === 0 ? this.state.address : this.state.changedAdress
    ...
    return (<div className="localposition-container">
      ...
      <div id="positionParent" style={{ "display": this.state.showMapContainer ? "block" : "none", ZIndex: "100" }}>
        <div className="position-list-map-container" ref="mapList">
          <div><Icon className="map-back-icon" type="left" onClick={this.handleCloseMapList} /><span className="map-title">定位</span></div>
