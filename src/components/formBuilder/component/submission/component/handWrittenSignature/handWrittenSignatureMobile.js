...
export default class HandWrittenSignatureMobile extends React.Component {
  ...
  DrawBegin = (e) => {
    ...
    this.Draw(e.pageX - this.refs.canvas.offsetLeft, e.pageY - this.refs.canvas.offsetTop, false)
  }
  Drawing = (e) => {
    if (this.state.mousePressed) {
      this.Draw(e.pageX - this.refs.canvas.offsetLeft, e.pageY - this.refs.canvas.offsetTop, true)
    }
  }
  DrawEnd = () => {
    this.setState({
  ...
  DrawBeginPh = (e) => {
    if (e.targetTouches.length === 1) {
      ...
      this.Draw(touch.pageX - this.refs.canvas.offsetLeft, touch.pageY - this.refs.canvas.offsetTop, false)
    }
  }
  DrawingPh = (e) => {
    if (e.targetTouches.length === 1) {
      let touch = e.targetTouches[0];
      if (this.state.mousePressed) {
        this.Draw(touch.pageX - this.refs.canvas.offsetLeft, touch.pageY - this.refs.canvas.offsetTop, true)
      }
    }
  }
  DrawEndPh = (e) => {
  ...
  render() {
    ...
    return (
      <>
        <Button className="signature-btn" onClick={() => {
        ...
        <div id="canvasParent" style={{ "display": "none" }}>
          <div className="canvas-header"><span className="canvas-back" onClick={(e) => this.handleCancel(e)} ><Icon type="left" /></span>手写签名</div>
          <canvas key={"myCanvasKey"} id="myCanvas" ref="canvas" onTouchStart={this.DrawBeginPh} onTouchMove={this.DrawingPh} onTouchEnd={this.DrawEndPh}></canvas>
          <div className="canvas-operate">
            <Button type="primary" onClick={this.ClearPath} >清空画板</Button>
